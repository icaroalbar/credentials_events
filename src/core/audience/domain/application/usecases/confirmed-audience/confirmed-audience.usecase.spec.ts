...
import { ConfirmedAudienceUseCase } from "./confirmed-audience.usecase";
...
describe("ConfirmedAudienceUseCase", () => {
  let confirmedAudienceUseCase: ConfirmedAudienceUseCase;

  beforeEach(() => {
    jest.clearAllMocks();
    confirmedAudienceUseCase = new ConfirmedAudienceUseCase(
      mockAudienceRepository
    );
  });

  it("should confirm an audience successfully", async () => {
    ...
    const result = await confirmedAudienceUseCase.execute(audience.getId());
